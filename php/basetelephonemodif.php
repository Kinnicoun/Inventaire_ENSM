<?php
			...
			if($_SESSION["membreid"]=="dsic"){
				...
				$requete = sprintf("SELECT Inventaire,Serie,Affectation,Fingarantie,Commentaire,InventaireENSM, constructeur.Nom as Constructeur,modele.Nom as Modele,Etat FROM telephone INNER JOIN modele ON(telephone.IdModele=modele.IdModele) INNER JOIN constructeur ON(telephone.IdConstructeur=constructeur.IdConstructeur) WHERE Etat!='Domaine' ORDER BY Inventaire ASC");
				$result = mysqli_query($link,$requete);
				echo ("<h1 align='center'>Modification T&eacute;l&eacute;phone</h1>");
				echo("<form action='selecttelephonemodif.php' method='post'>");
		...
		else
		{
			...
			$site=$_SESSION["site"];
			$link = mysqli_connect(serv(),utili(),pass(),bdd()) or die("Error " . mysqli_error($link));
				$requete = sprintf("SELECT Inventaire,Serie,Affectation,Fingarantie,Commentaire,InventaireENSM, constructeur.Nom as Constructeur,modele.Nom as Modele,Etat FROM telephone INNER JOIN modele ON(telephone.IdModele=modele.IdModele) INNER JOIN constructeur ON(telephone.IdConstructeur=constructeur.IdConstructeur) WHERE CodeSite='$site' AND Etat!='Domaine' ORDER BY Inventaire ASC");
				$result = mysqli_query($link,$requete);
				echo("<form action='selecttelephonemodif.php' method='post'>
						<table>
							<caption> Modification Téléphone </caption>
							<thead>
								<tr>
