<?php
	...
	echo("
		<div class='FormContain'>
			<div class='form'>
				<div class='form-heading'> Ajout &Eacute;cran </div>
				<form action='finajoutecran.php' method='post'>
				...
				<label><span>Fin de Garantie </span><input type='date'  class='input-field' size='15' name='fingarantie' value='aaaa-mm-jj'/></label>
		...
		$requete = sprintf("SELECT C.IdConstructeur, C.Nom FROM constructeur C inner join ecran E on (C.IdConstructeur = E.IdConstructeur) group by C.IdConstructeur, C.nom ");
		...
		$link = mysqli_connect(serv(),utili(),pass(),bdd()) or die("Error " . mysqli_error($link));
		$requete2 = sprintf("SELECT M.IdModele, M.Nom FROM Modele M inner join ecran E on (M.IdModele = E.IdModele) group by M.IdModele, M.nom ");
		...
		echo("
				...
				<label><span>Taille </span><input type='number' class='input-field' size='15' name='taille'/></label> 
