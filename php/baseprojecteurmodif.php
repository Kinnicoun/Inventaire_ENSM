<?php
		...
		if($_SESSION["membreid"]=="dsic"){
				...
				$requete = sprintf("SELECT Inventaire,Serie,Affectation,Fingarantie,Commentaire,InventaireENSM,constructeur.Nom as Constructeur,modele.Nom as Modele,Etat FROM videoprojecteur INNER JOIN modele ON(videoprojecteur.IdModele=modele.IdModele) INNER JOIN constructeur ON(videoprojecteur.IdConstructeur=constructeur.IdConstructeur) WHERE Etat!='Domaine' ORDER BY Inventaire ASC");
				$result = mysqli_query($link,$requete);
				echo ("<h1 align='center'>Modification Vid&eacute;o Projecteur</h1>");
				echo("<form action='selectprojecteurmodif.php' method='post'>");
				...
				while($row = mysqli_fetch_array($result)){
					echo("<tr>");
					echo("<td > <input type='radio' name='id[]' value='$row[Inventaire]'/></td>");
		...
		else
		{
			...
			$site=$_SESSION["site"];
			$link = mysqli_connect(serv(),utili(),pass(),bdd()) or die("Error " . mysqli_error($link));
				$requete = sprintf("SELECT Inventaire,Serie,Affectation,Fingarantie,Commentaire,InventaireENSM,constructeur.Nom as Constructeur,modele.Nom as Modele,Etat FROM videoprojecteur INNER JOIN modele ON(videoprojecteur.IdModele=modele.IdModele) INNER JOIN constructeur ON(videoprojecteur.IdConstructeur=constructeur.IdConstructeur) WHERE CodeSite='$site' AND Etat!='Domaine' ORDER BY Inventaire ASC");
				$result = mysqli_query($link,$requete);
				echo("<form action='selectprojecteurmodif.php' method='post'>
						<table>
							<caption> Modification Projecteur </caption>
							<thead>
								<tr>
