<?php
		include("../connexion.php");
			...
			catch (PDOException $e)
			{
				echo 'Could not connect : ' . $e->getMessage();
						...
						echo("<form action='finajoutuc.php' method='post'>
				<div class='FormContain'>
					<div class='form'>
						<div class='form-heading'> Ajout Unit&eacute; centrale </div>
		...
		$requete = sprintf("SELECT C.IdConstructeur, C.Nom FROM constructeur C inner join uc on (C.IdConstructeur = uc.IdConstructeur) group by C.IdConstructeur, C.nom");
		...
		$link = mysqli_connect(serv(),utili(),pass(),bdd()) or die("Error " . mysqli_error($link));
		$requete2 = sprintf("SELECT M.IdModele, M.Nom FROM Modele M inner join uc on (M.IdModele = uc.IdModele) group by M.IdModele, M.nom ");
