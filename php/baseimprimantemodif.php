<?php
	...
	session_start();
		...
		if($_SESSION["membreid"]=="dsic"){
				...
				$requete = sprintf("SELECT Inventaire, Affectation, Fingarantie, Serie, IP, Netbios,Commentaire,InventaireENSM,constructeur.Nom as Constructeur,modele.Nom as Modele,Etat FROM imprimante INNER JOIN modele ON(imprimante.IdModele=modele.IdModele) INNER JOIN constructeur ON(imprimante.IdConstructeur=constructeur.IdConstructeur) WHERE Etat!='Domaine' ORDER BY Inventaire ASC");
				$result = mysqli_query($link,$requete);
				echo ("<h1 align='center'>Modification Imprimante</h1>");
				echo("<form action='selectimprimantemodif.php' method='post'>");
				...
				echo("<td align='center' width='200px'><b> IP </b></td>");
				echo("<td align='center' width='200px'><b> Netbios </b></td>");
				...
				while($row = mysqli_fetch_array($result)){
					...
					echo("<td align='center' width='200px'> $row[IP] </td>");
					echo("<td align='center' width='200px'> $row[Netbios] </td>");
		...
		else
		{
			...
			$site=$_SESSION["site"];
			$link = mysqli_connect(serv(),utili(),pass(),bdd()) or die("Error " . mysqli_error($link));
			$requete = sprintf("SELECT Inventaire, Affectation, Fingarantie, Serie, IP, Netbios,Commentaire,InventaireENSM,constructeur.Nom as Constructeur,modele.Nom as Modele,Etat FROM imprimante INNER JOIN modele ON(imprimante.IdModele=modele.IdModele) INNER JOIN constructeur ON(imprimante.IdConstructeur=constructeur.IdConstructeur) WHERE CodeSite='$site' AND Etat!='Domaine' ORDER BY Inventaire ASC");
			$result = mysqli_query($link,$requete);
			echo("<form action='selectimprimantemodif.php' method='post'>
						<table>
							<caption> Modification Imprimante </caption>
							<thead>
								<tr>
							...
							<tfoot>
								<tr>
									...
									<th align='center' colspan='9' ><input type='submit' value='Modifier' name='valider'/> </th>	
								</tr>
							</tfoot>							
					");
				while($row = mysqli_fetch_array($result)){
