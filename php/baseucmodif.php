<?php
			...
			if($_SESSION["membreid"]=="dsic"){
				...
				$requete = sprintf("SELECT Inventaire,Serie,Affectation,Fingarantie,Processeur,MemoireviveGo,DisquedurGo,OS,Commentaire,InventaireENSM,constructeur.Nom as Constructeur,modele.Nom as Modele,Etat FROM uc INNER JOIN modele ON(uc.IdModele=modele.IdModele) INNER JOIN constructeur ON(uc.IdConstructeur=constructeur.IdConstructeur) WHERE Etat!='Domaine' ORDER BY Inventaire ASC");
				$result = mysqli_query($link,$requete);
				echo ("<h1 align='center'>Modification Unit&eacute; Centrale</h1>");
				echo("<form action='selectucmodif.php' method='post'>");
		...
		else
		{
			...
			$site=$_SESSION["site"];
			$link = mysqli_connect(serv(),utili(),pass(),bdd()) or die("Error " . mysqli_error($link));
				$requete = sprintf("SELECT Inventaire,Serie,Affectation,Fingarantie,Processeur,MemoireviveGo,DisquedurGo,OS,Commentaire,InventaireENSM,constructeur.Nom as Constructeur,modele.Nom as Modele,Etat FROM uc INNER JOIN modele ON(uc.IdModele=modele.IdModele) INNER JOIN constructeur ON(uc.IdConstructeur=constructeur.IdConstructeur)WHERE CodeSite='$site' AND Etat!='Domaine' ORDER BY Inventaire ASC");
				$result = mysqli_query($link,$requete);
				echo ("
				<form action='selectucmodif.php' method='post'>
					<table>
						<caption> Modification Unité Centrale </caption>
							<thead>
								<tr>
