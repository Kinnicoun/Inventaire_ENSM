<?php
		...
		echo("<form action='finajoutportable.php' method='post'>
				<div class='FormContain'>
					<div class='form'>
						<div class='form-heading'> Ajout Portable </div>
		...
		$requete = sprintf("SELECT C.IdConstructeur, C.Nom FROM constructeur C inner join portable P on (C.IdConstructeur = P.IdConstructeur) group by C.IdConstructeur, C.nom ");
		...
		$link = mysqli_connect(serv(),utili(),pass(),bdd()) or die("Error " . mysqli_error($link));
		$requete2 = sprintf("SELECT M.IdModele, M.Nom FROM Modele M inner join portable P on (M.IdModele = P.IdModele) group by M.IdModele, M.nom  ");
		...
		echo("									<label><span>État </span><select class='wrapper-dropdown' name='etat'>");
