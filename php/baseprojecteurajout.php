<?php
		...
		echo("<form action='finajoutprojecteur.php' method='post'>
				<div class='FormContain'>
					<div class='form'>
						<div class='form-heading'> Ajout Projecteur </div>
						");
		echo("									<label><span>Inventaire </span><input type='text' class='input-field' size='15' name='inventaire'/></label>"); 
		echo("									<label><span>Série </span><input type='text' class='input-field' size='15' name='serie'/></label>"); 
		echo("									<label><span>Affectation </span><input type='text' class='input-field' size='15' name='affectation'/></label>");
		echo("									<label><span>Fin de garantie</span><input type='date' class='wrapper-dropdown' name='fingarantie' value='aaaa-mm-jj'/></label>");
		echo("									<label><span>Constructeur </span><select class='wrapper-dropdown' name='constructeur'>");
		
		$link = mysqli_connect(serv(),utili(),pass(),bdd()) or die("Error " . mysqli_error($link));
		$requete = sprintf("SELECT C.IdConstructeur, C.Nom FROM constructeur C inner join videoprojecteur V on (C.IdConstructeur = V.IdConstructeur) group by C.IdConstructeur, C.nom ");
		...
		echo"									</select></label>";
		echo"									<label><span>Modèle </span><select class='wrapper-dropdown' name='modele'>";
		
		$link = mysqli_connect(serv(),utili(),pass(),bdd()) or die("Error " . mysqli_error($link));
		$requete2 = sprintf("SELECT M.IdModele, M.Nom FROM Modele M inner join videoprojecteur V on (M.IdModele = V.IdModele) group by M.IdModele, M.nom ");
		$result2 = mysqli_query($link,$requete2);
		while($row2 = mysqli_fetch_array($result2, MYSQLI_ASSOC))
		{
			echo("										<option value='$row2[IdModele]'>$row2[Nom]</option>");
		...
		echo("									<label><span>Inventaire ENSM </span><input type='text' class='input-field' size='15' name='inventaireensm'/></label>");
		echo("									<label><span>Etat </span><select class='wrapper-dropdown' name='etat'>");
