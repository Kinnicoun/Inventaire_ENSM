<?php
		include("../connexion.php");
			...
			catch (PDOException $e)
			{
				...
				echo("<form action='finajoutswitch.php' method='post'>
				<div class='FormContain'>
					<div class='form'>
						<div class='form-heading'> Ajout Borne Wifi </div>
							...
							<label><span>Adresse MAC </span><input type='text' class='input-field' size='15' name='adresse'/></label>
							<label><span>Affectation </span><input type='text' class='input-field' size='15' name='affectation'/></label>
							<label><span>Numéro prise </span><input type='text' class='input-field' size='15' name='numprise'/></label>
							<label><span>Fin de garantie </span><input type='date' class='wrapper-dropdown' name='fingarantie' value='aaaa-mm-jj'/></label>
							<label><span>Positionnement </span><input type='text' class='input-field' size='15' name='positionnement'/></label>
							<label><span>Constructeur </span><select class='wrapper-dropdown' name='constructeur'></label>
							");
		
		$link = mysqli_connect(serv(),utili(),pass(),bdd()) or die("Error " . mysqli_error($link));
		$requete = sprintf("SELECT C.IdConstructeur, C.Nom FROM constructeur C inner join wifi on (C.IdConstructeur = wifi.IdConstructeur) group by C.IdConstructeur, C.nom");
		...
		$link = mysqli_connect(serv(),utili(),pass(),bdd()) or die("Error " . mysqli_error($link));
		$requete2 = sprintf("SELECT M.IdModele, M.Nom FROM Modele M inner join wifi on (M.IdModele = wifi.IdModele) group by M.IdModele, M.nom ");
